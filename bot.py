...
def main():
    ...
    if state.paper_engine:
        ...
        # 🛡️ Live-mode restart resume: pick monitoring back up for
        # whatever positions were already open before this restart, so the
        ...
        # a reasonable default for real money.) The automation engine can
        # track any number of concurrently-open live positions, so every
        # position found gets resumed — there's no "only if exactly one"
        ...
        if not PAPER_MODE:
            ...
            if open_live_positions:
                ...
                pos_lines = "\n".join(
                    f"  • {b(p['symbol'])} — ${p.get('amount_usd', 0):.0f} × {p.get('leverage', '?')}x — "
                    f"{code(p['id'][:8] + '…')}"
                    for p in open_live_positions
                )
                resume_msg = (
                    f"⚠️ {b(f'BOT RESTART — RESUMED MONITORING {len(open_live_positions)} LIVE POSITION(S)')}\n\n"
                    f"{pos_lines}\n\n"
                    f"{i('Automation engine is watching these again (hedge guard, exit logic, delisting guard).')}"
                )
                log.warning("Resumed LIVE monitoring for %d position(s) after restart", len(open_live_positions))
                if NOTIFY_CHAT_ID:
                    try:
                        import requests
                        requests.post(
